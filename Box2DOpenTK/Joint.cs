...
namespace Box2DOpenTK
{
    public class Joint
    {
        public static int SecretCookie = 1152023;

        DistanceJointDef DefaultDistanceJointDef()
        {
            DistanceJointDef def = new DistanceJointDef();
            def.length = 1.0f;
            def.maxLength = World.lengthUnitsPerMeter * 100000.0f;
            def.internalValue = 1152023;
            return def;
        }

        MotorJointDef DefaultMotorJointDef()
        {
            MotorJointDef def = new MotorJointDef();
            def.maxForce = 1.0f;
            def.maxTorque = 1.0f;
            def.correctionFactor = 0.3f;
            def.internalValue = SecretCookie;
            return def;
        }

        MouseJointDef DefaultMouseJointDef()
        {
        ...
        PrismaticJointDef b2DefaultPrismaticJointDef()
        {
            PrismaticJointDef def = new PrismaticJointDef();
            def.localAxisA = new OpenTK.Mathematics.Vector2(1.0f, 0.0f);
            def.internalValue = SecretCookie;
            return def;
        }

        RevoluteJointDef b2DefaultRevoluteJointDef()
        {
        ...
        WeldJointDef DefaultWeldJointDef()
        {
        ...
        WheelJointDef DefaultWheelJointDef()
        {
        ...
        static Joint GetJointFullId(ref World world, JointId jointId)
        {
            int id = jointId.index1 - 1;
            CheckIndex(world.jointArray, id);
            Joint joint = world.jointArray[id];
            CheckIndex(world.solverSetArray, joint.setIndex);
            if (joint.revision != jointId.revision) throw new Exception("joint.revision != jointId.revision");
            return joint;
        }

        Joint GetJoint(ref World world, int jointId)
        {
            CheckIndex(world.jointArray, jointId);
            Joint joint = world.jointArray[jointId];
            return joint;
        }

        JointSim GetJointSim(ref World world, Joint joint)
        {
            CheckIndex(world.solverSetArray, joint.setIndex);

            if (joint.setIndex == awakeSet)
            {
